class AzureOpenaiService
    ...
    def analyze_image(image_data:, prompt:)
      file_url = "data:image/jpeg;base64,#{Base64.strict_encode64(image_data)}"
      messages = [
        { role: "user", content: [
      ...
      chat(messages: messages, model: ENV['AZURE_OPENAI_VISION_DEPLOYMENT_NAME'])
    end
    ...
    def handle_error(error)
      ...
      if error.message.include?('429')
        raise RateLimitExceededError, "Azure OpenAI APIのレート制限に達しました。しばらく待ってから再試行してください。"
      else
